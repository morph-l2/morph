...
use revm::{
    ...
    primitives::{
        address, bytes, keccak256, AccountInfo, AuthorizationList as RevmAuthorizationList,
        BlockEnv, Bytecode, Bytes, Env, SpecId, TxEnv, B256, KECCAK_EMPTY, POSEIDON_EMPTY, U256,
    },
    Database,
};
use sbv_primitives::{
    types::BlockTrace, zk_trie::ZkMemoryDb, Address, Block, SignedAuthorization, Transaction,
    TxTrace,
};
...
impl EvmExecutor<'_> {
    ...
    fn handle_block_inner<T: Block>(&mut self, l2_trace: &T) -> Result<(), VerificationError> {
        ...
        for (idx, tx) in l2_trace.transactions().enumerate() {
            ...
            println!("tx.fee_token_id(): {:?}", tx.fee_token_id());
            env.tx = TxEnv {
            ...
            println!("env.tx.caller: {:?}", env.tx.caller);
...
fn exec_alt_fee_txn(
    ...
    let mut evm = revm::Evm::builder().with_db(&mut cache_db).build();
