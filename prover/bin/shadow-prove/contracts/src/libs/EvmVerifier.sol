...
contract EvmVerifier is SP1Verifier {
    ...
    bytes32 public shardVkey;

    /// @notice The verification key for the morph executor program.
    bytes32 public aggVkey;

    constructor(bytes32 _shardVkey, bytes32 _aggVkey) {
        shardVkey = _shardVkey;
        aggVkey = _aggVkey;
    }
    ...
    function verifyBatchProof(bytes calldata proof, bytes calldata publicValues) public view {
        this.verifyProof(aggVkey, abi.encode(publicValues, shardVkey), proof);
    }
}
