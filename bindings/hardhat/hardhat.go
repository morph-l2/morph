...
// New creates a new `Hardhat` struct that fork from New(network string, artifacts, deployments []string)

func New4SingleDeployment(network string, deployment string) (*Hardhat, error) {
	hh := &Hardhat{
		network:         network,
		DeploymentPaths: []string{deployment},
	}

	if err := hh.initSingleDeployment(); err != nil {
		return nil, err
	}
	return hh, nil
}
...
// init is called in the constructor and will cache required files to disk.
func (h *Hardhat) initSingleDeployment() error {
	h.amu.Lock()
	defer h.amu.Unlock()
	h.dmu.Lock()
	defer h.dmu.Unlock()
	...
	for _, deploymentPath := range h.DeploymentPaths {
		...
		for _, entry := range results {
			...
			deployment.Name = entry["name"].(string)
			deployment.Address = common.HexToAddress(entry["address"].(string))
			deployment.Receipt.BlockNumber = uint(entry["number"].(float64))
