...
export const SubmitterRegister = async (
    ...
    const ownerView = await hre.ethers.getContractAt(ContractFactoryName.Submitter, proxyAddress, ownerSigner);
    const account = await submitterSigner.getAddress();
    if ((await ownerView.owner()).toLowerCase() !== (await ownerSigner.getAddress()).toLowerCase()) {
