...
task("check-l2").setAction(async (taskArgs, hre) => {
    ...
    for (let i = 0; i < ContractAddresses.length; i++) {
        ...
        const temp = new ethers.Contract(proxy.address, proxy.interface, proxy.provider)
        ...
        console.log(
            `implementation is equal ProxyAdmin: ${adminAddr.toLowerCase() == predeploys.ProxyAdmin.toLowerCase()}`
        )
    }
})

task("check-l2-status").setAction(async (taskArgs, hre) => {
    ...
    console.log(
        `L2ToL1MessagePasser params check \n root set status ${
            messageRoot == "0x27ae5ba08d7291c96c8cbddcc148bf48a6d68c7974b94356f53754ef6171d757"
        }`
    )
    console.log("-----------------------------------\n")
    ...
    console.log(
        `L2GatewayRouter params check \n owner ${owner} \n ETHGateway set ${
            ethGateway == predeploys.L2ETHGateway
    ...
    console.log(
        `Sequencer params check \n owner ${owner} \n staking ${
            stakingAddr == predeploys.L2Staking
    ...
    console.log(
        `L2CrossDomainMessenger params check \n owner ${owner} \n paused ${
            paused == false
    ...
    console.log(
        `L2StandardERC20Gateway params check \n owner ${owner} \n tokenFactory ${
            tokenFactory == predeploys.MorphStandardERC20Factory
        } \n router ${router == predeploys.L2GatewayRouter} \n messenger ${
            messenger == predeploys.L2CrossDomainMessenger
        } \n counterpart ${counterpart}`
    )
    ...
    console.log(
        `MorphStandardERC20 params check \n name ${name} \n symbol ${symbol} \n decimals ${decimals} \n gateway ${gateway} \n counterpart ${counterpart}`
    )
    console.log("-----------------------------------\n")
    ...
    console.log(
        `MorphStandardERC20Factory params check \n owner ${
            owner == predeploys.L2StandardERC20Gateway
...
task("deposit-l1-eth").setAction(async (taskArgs, hre) => {
    ...
    const res = await router["depositETH(uint256,uint256)"](hre.ethers.utils.parseEther("1"), 110000, {
...
task("withdraw-l2-eth").setAction(async (taskArgs, hre) => {
...
task("deposit-l1-gateway-eth").setAction(async (taskArgs, hre) => {
    ...
    const res = await contract["depositETH(uint256,uint256)"](hre.ethers.utils.parseEther("1"), 110000, {
        value: hre.ethers.utils.parseEther("1.1"),
    })
...
task("deploy-token").setAction(async (taskArgs, hre) => {
...
task("deposit-erc20-token")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        res = await router["depositERC20(address,uint256,uint256)"](l1token.address, allowance, 420000, {
...
task("withdraw-l2-eth").setAction(async (taskArgs, hre) => {
    ...
    const res = await contract["withdrawETH(uint256,uint256)"](ethers.utils.parseEther("1"), 0, {
...
task("getSequencerAddresses").setAction(async (taskArgs, hre) => {
...
task("l2factory-deploy").setAction(async (taskArgs, hre) => {
