...
contract BatchHeaderCodecTest {
    ...
    function getSequencerSetVerifyHash(
        ...
        (uint256 batchPtr, ) = BatchHeaderCodecV1.loadAndValidate(_batchHeader);
        return BatchHeaderCodecV1.getSequencerSetVerifyHash(batchPtr);
    }

    function getParentBatchHash(bytes calldata _batchHeader) public pure returns (bytes32 _parentBatchHash) {
        (uint256 batchPtr, ) = BatchHeaderCodecV1.loadAndValidate(_batchHeader);
        return BatchHeaderCodecV1.getParentBatchHash(batchPtr);
    }

    function computeBatchHash(bytes calldata _batchHeader) public pure returns (bytes32 batchHash) {
        (uint256 batchPtr, uint256 length) = BatchHeaderCodecV1.loadAndValidate(_batchHeader);
        return BatchHeaderCodecV1.computeBatchHash(batchPtr, length);
    }
    ...
    function computeBatchHashWithData(BatchHeaderData calldata data) public pure returns (bytes32 batchHash) {
        ...
        BatchHeaderCodecV1.storeVersion(_batchPtr, data.version);
        BatchHeaderCodecV1.storeBatchIndex(_batchPtr, data.batchIndex);
        BatchHeaderCodecV1.storeL1MessagePopped(_batchPtr, data.l1MessagePopped);
        BatchHeaderCodecV1.storeTotalL1MessagePopped(_batchPtr, data.totalL1MessagePopped);
        BatchHeaderCodecV1.storeDataHash(_batchPtr, data.dataHash);

        BatchHeaderCodecV1.storeBlobVersionedHash(_batchPtr, data.blobVersionedHash);
        BatchHeaderCodecV1.storePrevStateHash(_batchPtr, data.prevStateHash);
        BatchHeaderCodecV1.storePostStateHash(_batchPtr, data.postStateHash);
        BatchHeaderCodecV1.storeWithdrawRootHash(_batchPtr, data.withdrawRootHash);
