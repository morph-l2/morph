...
interface IDistribute {
     ...
     */
    function initialize(address morphToken_, address record_, address gov_, address stake_) external;

    function notify(uint256 blockTime, uint256 blockNumber) external;

    function notifyUnDelegate(uint256 epochIndex, address sequencer, address account, uint256 blockNumber) external;

    function notifyDelegate(uint256 epochIndex, address sequencer, address account, uint256 amount, uint256 blockNumber) external;
