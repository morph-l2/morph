...
contract L2Sequencer is Initializable, IL2Sequencer, Sequencer {
    ...
    ) Sequencer(payable(Predeploys.L2_CROSS_DOMAIN_MESSENGER), _otherSequencer) {
        L2_SUBMITTER_CONTRACT = Predeploys.L2_SUBMITTER;
    }

    function initialize(Types.SequencerInfo[] memory _sequencers) public initializer {
        ...
        sequencerHistory[0] = Types.SequencerHistory(sequencerAddresses, block.timestamp);
    }
    ...
    function updateSequencers(
        ...
        sequencerHistory[version] = Types.SequencerHistory(sequencerAddresses, block.timestamp);
    ...
    function inSequencersSet(
        ...
        if (previous && currentVersion > 0) {
            for (
                uint256 i = 0;
                i < sequencerHistory[currentVersion - 1].sequencerAddresses.length;
                i++
            ) {
                if (checkAddr == sequencerHistory[currentVersion - 1].sequencerAddresses[i]) {
        ...
        for (uint256 i = 0; i < sequencerHistory[currentVersion].sequencerAddresses.length; i++) {
            if (checkAddr == sequencerHistory[currentVersion].sequencerAddresses[i]) {
    ...
    function sequencersLen(bool previous) external view returns (uint256, uint256) {
        ...
        return (sequencerHistory[currentVersion].sequencerAddresses.length, currentVersion);
    }
}
