...
} from "../universal/MorphMintableERC20.sol";
...
contract StandardBridgeTester is StandardBridge {
    ...
    function isMorphMintableERC20(address _token) external view returns (bool) {
        return _isMorphMintableERC20(_token);
    }
 ...
 * @notice Simple implementation of the legacy MorphMintableERC20.
 */
contract LegacyMintable is ERC20, ILegacyMintableERC20 {
     ...
     *         it is how the actual legacy Morph mintable token does the
...
contract StandardBridge_Stateless_Test is CommonTest {
    ...
    MorphMintableERC20 internal mintable;
    ...
    function setUp() public override {
        ...
        mintable = new MorphMintableERC20({
            _bridge: address(0),
    ...
    /**
     * @notice Test coverage for identifying MorphMintableERC20 tokens.
     *         This function should return true for both modern and legacy
     *         MorphMintableERC20 tokens and false for any accounts that
     *         do not implement the interface.
     */
    function test_isMorphMintableERC20_succeeds() external {
        // Both the modern and legacy mintable tokens should return true
        assertTrue(bridge.isMorphMintableERC20(address(mintable)));
        assertTrue(bridge.isMorphMintableERC20(address(legacy)));
        // A regular ERC20 should return false
        assertFalse(bridge.isMorphMintableERC20(address(erc20)));
        // Non existent contracts should return false and not revert
        assertEq(address(0x20).code.length, 0);
        assertFalse(bridge.isMorphMintableERC20(address(0x20)));
    }
