...
import {Portal_Initializer, Reverter, CallerCaller, CommonTest} from "./CommonTest.t.sol";
...
contract CrossDomainMessenger_BaseGas_Test is Portal_Initializer {
     ...
     *         or equal to the minimum gas limit value on the MorphPortal.
     *         This guarantees that the messengers will always pass sufficient
     *         gas to the MorphPortal.
     */
...
contract ExternalRelay is CommonTest {
    ...
    function externalCallWithMinGas() external payable {
        for (uint256 i = 0; i < 10; i++) {
            ...
            unchecked {
                _innerSender = address(
                    uint160(uint256(uint160(fuzzedSender)) + i)
                );
            }
    ...
    function getCallData() public pure returns (bytes memory) {
        return
            abi.encodeWithSelector(
                ExternalRelay.externalCallWithMinGas.selector
            );
    }
 ...
 */
contract CrossDomainMessenger_RelayMessage_Test is Portal_Initializer {
    // Storage slot of the l2Sender
    uint256 constant senderSlotIndex = 54;
    ...
    /**
     * @dev This test mocks an MorphPortal call to the L1CrossDomainMessenger via
     ...
     */
    function testFuzz_relayMessageReenter_succeeds(
        address _sender,
        uint256 _gasLimit
    ) external {
