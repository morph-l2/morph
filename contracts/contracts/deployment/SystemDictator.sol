...
contract SystemDictator is OwnableUpgradeable {
    ...
    function step5() public onlyOwner step(5) {
        // Upgrade and initialize the MorphPortal.
        config.globalConfig.proxyAdmin.upgradeAndCall(
            payable(config.proxyAddressConfig.MorphPortalProxy),
            address(config.implementationAddressConfig.MorphPortalImpl),
            abi.encodeCall(
                MorphPortal.initialize,
                (
                    MorphPortalDynamicConfig,
                    config.proxyAddressConfig.l1CrossDomainMessengerProxy
                )
            )
        );
        ...
        config.globalConfig.proxyAdmin.upgrade(
            payable(
                config.proxyAddressConfig.MorphMintableERC20FactoryProxy
            ),
            address(
                config
                    .implementationAddressConfig
                    .MorphMintableERC20FactoryImpl
            )
        );
