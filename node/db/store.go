...
// DerivationL1Block stores the (number, hash) pair for an L1 block that
// derivation has scanned for commit batch logs. SPEC-005 §4.7.6 reorg
// detection compares saved hashes against fresh L1 reads; on mismatch the
// derivation cursor is rewound.
...
// inclusive. Missing entries are skipped silently; the slice is dense over
// the heights actually present.
...
// DeleteDerivationL1BlocksFrom drops every saved L1 block record at height
// >= the given height. Used by handleL1Reorg to clear hashes that are no
// longer canonical so subsequent polls record the new chain afresh.
