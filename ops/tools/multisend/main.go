...
func main() {
	...
	var count int
	for {
		if count == 1000000 || time.Now().Sub(start) > duration {
		...
		for _, sender := range senderPks {
			go func(s *ecdsa.PrivateKey) {
				...
				fmt.Println("sent tx, count: ", count)
			}(sender)
		}
		count++
...
func distribute(client *ethclient.Client) []*ecdsa.PrivateKey {
	fundAddr := crypto.PubkeyToAddress(fundPrivKey.PublicKey)
	balance, err := client.BalanceAt(context.Background(), fundAddr, nil)
	if err != nil {
		panic(err)
	}
	fmt.Println("fundBalance: ", balance.String())
